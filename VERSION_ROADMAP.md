...
| v0.11 | Source Expansion | Fix 3 broken Tier 1 scrapers (Defense One, Breaking Defense, IISS). Add 3-5 new free sources (CSIS, Reuters, Al Jazeera, The War Zone + 1 more) to reach 8-10 total working scrapers. All sources validated with article extraction, region tagging, and JSON output. Pipeline tested end-to-end with expanded source set. | 12-16h | - | pending |
| v0.12 | Global Briefing | New /briefing/global backend endpoint that synthesizes articles from all regions into a single cross-regional intelligence overview. Mobile ALL tab updated to fetch from this endpoint instead of showing 4 separate regional cards. Global briefing cached weekly with the other regional briefings. | 6h | - | pending |
